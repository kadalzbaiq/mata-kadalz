...
This guide covers running `mata-kadalz` on Linux. Two parts:
...
1. Install **llama.cpp** following its official docs for your distro:
   https://github.com/ggml-org/llama.cpp#readme (releases:
   https://github.com/ggml-org/llama.cpp/releases, e.g.
   `llama-b10448-bin-ubuntu-x64.tar.gz` for x86_64 Ubuntu; some distros ship it
   as a package) — install the `llama-server` binary, do NOT install it via
   this repo.
...
1. Install Python >=3.11: `sudo apt install python3 python3-venv` (Debian/Ubuntu) or your distro equivalent.
